...
#pragma omp parallel for private(index_X,index_Y,j) num_threads(4)
   	for(i = 0; i<n; i++){
   		for(j = 0; j < Ones; j++){
...
#pragma omp parallel for reduction(+:sumWeights) num_threads(4)
   	for(i = 0; i < n; i++)
...
#pragma omp parallel for reduction(+:estimate_x,estimate_y) num_threads(4)
	for(i = 0; i < n; i++){
